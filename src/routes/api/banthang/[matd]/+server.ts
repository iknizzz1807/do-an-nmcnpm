...
import {
  ...
  selectBanThang,
  updateBanThang,
} from "$lib/server/db/functions/BanThang";
...
export const GET: RequestHandler = async ({ params, locals }) => {
  ...
  if (!isNumber(maTD)) throw new Error("MaTD phải là số");
...
export const POST: RequestHandler = async ({
  ...
  try {
    ...
    if (params === "") throw new Error("Param hiện là rỗng");
    console.log(data);
    let banThang: UpdateBanThang = {
      oldBanThang:
        (data.oldBanThang ?? null) === null
          ? null
          : {
    ...
    const thoiDiemGhiBanToiThieu = (await selectThamSo(
      "thoiDiemGhiBanToiThieu"
    ))!!;
    const thoiDiemGhiBanToiDa = (await selectThamSo("thoiDiemGhiBanToiDa"))!!;
    if (
      banThang.newBanThang.thoiDiem > thoiDiemGhiBanToiDa ||
      banThang.newBanThang.thoiDiem < thoiDiemGhiBanToiThieu
    ) {
      throw new Error(
        "Thời điểm vượt quá thời điểm ghi bàn tối đa " +
          thoiDiemGhiBanToiDa +
          " hoặc thời điểm ghi bàn tối thiểu" +
          thoiDiemGhiBanToiThieu
      );
    }

    if (
      !isCauThuInTranDau(banThang.newBanThang.maTD, banThang.newBanThang.maCT)
    ) {
